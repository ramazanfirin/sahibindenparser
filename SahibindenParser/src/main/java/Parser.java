...
public class Parser {
	...
	if(locationElements.size()>0){
		...
		if(b.size()>0){
			sehir  =encode(b.get(0).html());
			ilce = encode(b.get(1).html());
			mah = encode(b.get(2).html());
	...
	if(informations.size()>0){
		...
		String[] temp = ilanTarihi.split(" ");
		ilanTarihi = temp[2];
		
		Element emlakTipiElement = b.get(2);
		emlakTipi  = emlakTipiElement.child(1).html();
		...
		String temps[] = odaSayisi.split("\\+");
		int total=0;
		for (int i = 0; i < temps.length; i++) {
			total = total+Integer.parseInt(temps[i]);
		}
		
		odaSayisi = String.valueOf(total);
	...
	out.println(fiyat+","+sehir+","+ilce+","+mah+","+ilanNo+","+ilanTarihi+","+emlakTipi+","+m2+","+odaSayisi+","+banyoSayisi+","+binaYasi+","+binaKatSayisi+","+bulunduguKat+","+isitma+","+kullanimDurumu+","+krediyeUygun+","+kimden+","+","+takas);
}
...
public static String encode(String str){
	String results = str;
	results=	StringEscapeUtils.unescapeHtml(str);
	return results;
}
}
